...
class SubmitCommentUseCase @Inject constructor(
    ...
    suspend operator fun invoke(postId: String, commentText: String): Result<Post, NetworkError> {
        ...
        return when(val result = homeRepository.submitComment(commentId, postId, commentText, user, userType)) {
            is Result.Success -> {
                    ...
                    try {
                        withTimeout(20000L) {
                            if (user is Person || (user is Company && user.id != post.userId)) {
                                Log.d("SubmitCommentUseCase", "Other users with comments result: ${post.userId}")
                            ...
                            val otherUsersWithCommentsResult = homeRepository.getUsersWhoCommentedByPostId(
                                postId,
                                post.userId
                            )
                            ...
                            if (otherUsersWithCommentsResult is Result.Success) {
                                ...
                                Log.d("SubmitCommentUseCase", "Other users with comments result: $otherUsersWithComments")
